...
class SwiftDataBackendSettings(BaseDataBackendSettings):
    """Represents the SWIFT data backend default configuration.
    ...
    Attributes:
        ...
        OBJECT_STORAGE_URL (str): The URL of the default target container.
...
class SwiftDataBackend(HistoryMixin, BaseDataBackend):
    ...
    def __init__(self, settings: settings_class = None):
        ...
        self.identity_api_version = settings.IDENTITY_API_VERSION

        self.tenant_id = settings.TENANT_ID
        self.tenant_name = settings.TENANT_NAME
        self.region_name = settings.REGION_NAME
        self.user_domain_name = settings.USER_DOMAIN_NAME
        self.project_domain_name = settings.PROJECT_DOMAIN_NAME
        self.object_storage_url = settings.OBJECT_STORAGE_URL
        self.default_container = settings.DEFAULT_CONTAINER
        ...
        self.connection = Connection(
            ...
            auth_version=self.identity_api_version,
        )
    ...
    @cached_property
    def options(self):
        """Returns the required options for the Swift Connection."""
        return {
    ...
    def _details(self, container: str, name: str):
        """Returns `name` object details from `container`."""
        try:
    ...
    def read(
        ...
        *,
        ...
        """Reads objects matching the `query` in the `target` container and yields them.
        ...
        logger.debug(
            "Getting object: %s from container: %s", query.query_string, target
        )
    ...
    def _read_raw(
        ...
        """Reads the `object` by line and yield bytes."""
        while chunk := obj.read(chunk_size):
            yield chunk

    # @staticmethod
    # def _write_raw(obj: IO, chunk: bytes) -> None:
    #     """Writes the `chunk` bytes to the file."""
    #     file.write(chunk)

    # def _write_dict(self, obj: IO, chunk: dict) -> None:
