...
class BaseXapiActivityTypeDefinition(BaseModelWithConfig):
...
class BaseXapiActivityTypeInteractionDefinition(BaseXapiActivityTypeDefinition):
    """Pydantic model for `Activity` type `definition` property.
...
class BaseXapiActivityType(BaseModelWithConfig):
...
class BaseXapiStatementRefType(BaseModelWithConfig):
...
BaseXapiUnnestedObject = Union[BaseXapiActivityType, BaseXapiStatementRefType]
