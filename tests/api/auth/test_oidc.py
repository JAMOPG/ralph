...
@responses.activate
@pytest.mark.parametrize("auth_mode", ["oidc"])
def test_api_auth_oidc_valid(
    auth_test_client, mock_discovery_response, mock_oidc_jwks, encoded_token
):
    ...
    assert response.json() == {"scopes": None, "username": "123|oidc"}


@responses.activate
@pytest.mark.parametrize("auth_mode", ["oidc"])
def test_api_auth_invalid_token(
    auth_test_client, mock_discovery_response, mock_oidc_jwks
):
...
@responses.activate
@pytest.mark.parametrize("auth_mode", ["oidc"])
def test_api_auth_invalid_discovery(auth_test_client, encoded_token):
