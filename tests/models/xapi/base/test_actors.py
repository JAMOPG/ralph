...
@custom_given(BaseXapiAgentWithMboxSha1SumIFI)
def test_models_xapi_base_actor_agent_with_mbox_sha1_sum_ifi_with_valid_field(
    field,
):
    """Tests a valid BaseXapiAgentWithMboxSha1SumIFI has the expected
    `mbox_sha1sum` regex.
    """
...
@custom_given(BaseXapiAgentWithMboxSha1SumIFI)
def test_models_xapi_base_actor_agent_with_mbox_sha1_sum_ifi_with_invalid_field(
    mbox_sha1sum, field
):
    """Tests an invalid `mbox_sha1sum` property in
    BaseXapiAgentWithMboxSha1SumIFI raises a `ValidationError`.
    """
    ...
    with pytest.raises(
        ...
        BaseXapiAgentWithMboxSha1SumIFI(**invalid_field)


@custom_given(BaseXapiGroupCommonProperties)
def test_models_xapi_base_actor_group_common_properties_with_valid_field(
    field,
):
