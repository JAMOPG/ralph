...
def test_backends_data_swift_data_backend_default_instantiation(monkeypatch, fs):
    """Tests the `SwiftDataBackend` default instantiation."""
    ...
    assert backend.user_domain_name == "Default"
    assert backend.project_domain_name == "Default"
    assert backend.auth_url == "https://auth.cloud.ovh.net/"
    assert backend.identity_api_version == "3"
    assert backend.locale_encoding == "utf8"


def test_backends_data_swift_data_backend_instantiation_with_settings(fs):
    """Tests the `SwiftDataBackend` instantiation with settings."""
    # pylint: disable=invalid-name
    fs.create_file(".env")
    ...
    assert backend.tenant_id == "tenant_id"
    assert backend.tenant_name == "tenant_name"
    assert backend.username == "username"
    assert backend.password == "password"
    assert backend.region_name == "region_name"
    assert backend.object_storage_url == "object_storage_url"
    assert backend.user_domain_name == "user_domain_name"
    assert backend.project_domain_name == "project_domain_name"
    assert backend.auth_url == "https://toto.net/"
    assert backend.identity_api_version == "2"
...
def test_backends_data_swift_data_backend_status_method_with_error_status(
    ...
    """Tests the `SwiftDataBackend.status` method, given a directory with wrong
    ...
    logger_name = "ralph.backends.data.swift"
    msg = f"Unable to connect to the Swift account: {error}"
...
def test_backends_data_swift_data_backend_list_method(
    ...
    """Tests that the `SwiftDataBackend.list` method argument should list
...
def test_backends_data_swift_data_backend_list_with_failed_details(
    ...
    """Tests that the `SwiftDataBackend.list` method with a failed connection
...
def test_backends_data_swift_data_backend_read_method_with_raw_output(
    ...
    """Tests the `SwiftDataBackend.read` method with `raw_output` set to `True`."""
...
def test_backends_data_swift_data_backend_read_method_without_raw_output(
    ...
    """Tests the `SwiftDataBackend.read` method with `raw_output` set to `False`."""
...
def test_backends_data_swift_data_backend_read_method_with_invalid_query(swift_backend):
    """Tests the `SwiftDataBackend.read` method given an invalid `query` argument should
...
def test_backends_data_swift_data_backend_read_method_with_ignore_errors(
    ...
    """Tests the `SwiftDataBackend.read` method with `ignore_errors` set to `True`,
...
def test_backends_data_swift_data_backend_read_method_without_ignore_errors(
    ...
    """Tests the `SwiftDataBackend.read` method with `ignore_errors` set to `False`,
...
def test_backends_data_swift_data_backend_read_method_with_failed_connection(
    ...
    """Tests the `SwiftDataBackend.read` method, given a `ClientException` raised by
...
def test_backends_data_swift_data_backend_write_method_with_file_exists_error(
    ...
    """Tests the `SwiftDataBackend.write` method, given a target matching an
    ...
    with pytest.raises(BackendException, match=msg):
        backend.write(target="object.gz", data=[b"foo"], operation_type=operation_type)
...
def test_backends_data_swift_data_backend_write_method_with_failed_connection(
    ...
    """Tests the `SwiftDataBackend.write` method, given a failed connection, should
...
def test_backends_data_swift_data_backend_write_method_with_invalid_operation(
    ...
    """Tests the `SwiftDataBackend.write` method, given an unsupported `operation_type`,
...
def test_backends_data_swift_data_backend_write_method_without_target(
    ...
    """Tests the `SwiftDataBackend.write` method, given no target, should write
