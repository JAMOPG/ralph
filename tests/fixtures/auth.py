...
@pytest.fixture
def auth_test_client(auth_mode, monkeypatch):
    """Select desired auth mode and return a TestClient."""
    if auth_mode not in [backend.value for backend in settings.AuthBackends]:
        auth_mode = "basic"

    monkeypatch.setenv("RALPH_RUNSERVER_AUTH_BACKEND", auth_mode)
